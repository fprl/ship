...
func CmdBoxApprovalLs(server string, jsonFlag bool) {
	...
	if err != nil || code != 0 {
		if err := sshResultError(server, stdout, stderr, code, err, "", "approvals failed", "ship box approval ls "+server); err != nil {
			utils.DieError(err, 1)
		}
	}
	fmt.Print(stdout)
...
func CmdBoxApprovalGrant(server, id string) {
	...
	if err != nil || code != 0 {
		if err := sshResultError(server, stdout, stderr, code, err, "", "grant failed", "ship box approval grant "+id+" "+server); err != nil {
			utils.DieError(err, 1)
		}
	}
	fmt.Print(stdout)
