...
func TestHelperSudoRegexRequiresServerSubtree(t *testing.T) {
	good := strings.TrimSpace(remoteprotocol.SudoersLine("deploy"))
	if !HelperSudoRe.MatchString(good) {
		t.Fatal("expected server subtree sudoers grant to match")
	}
	if HelperSudoRe.MatchString(strings.Replace(good, "/usr/local/bin/ship server --client-version * gc *, ", "", 1)) {
