...
case "$1" in
  ps) printf '%s\n' '[{"Names":["active"],"State":"running","Labels":{"ship.app":"api","ship.env":"production","ship.release":"abcdef1","ship.activation":"abcdef1-a1b2"}},{"Names":["old-running"],"State":"running","Labels":{"ship.app":"api","ship.env":"production","ship.release":"old1111","ship.activation":"old1111-a1b2"}},{"Names":["failed"],"State":"exited","Labels":{"ship.app":"api","ship.env":"production","ship.release":"old1111","ship.activation":"old1111-a1b2"}}]' ;;
...
`)
	...
	if !strings.Contains(string(log), "rm -f failed") || strings.Contains(string(log), "old-running") || !strings.Contains(string(log), "rmi "+identity.ImageTag("api", "production", "img-dead111")) {
